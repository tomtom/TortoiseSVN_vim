...
" s:GetCmdLine(command, ?filename)
fun! s:GetCmdLine(command, ...)
...
fun! TortoiseExec(command, ...)
    ...
    let cmd = s:GetCmdLine(a:command, fname)
    if cmd != ''
        if a:0 >= 1
            let extra = a:1
            let cmd   = cmd .' '. extra
        endif
        call s:ExecCommand(cmd)
    endif
    if a:command == 'commit'
...
- Make s:CanonicFileName(fname) &shell sensible
